<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Laporan Kehadiran Siswa') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-material-card title="Laporan Kehadiran Siswa" icon="assignment" color="purple">
                
                <!-- Filters -->
                <form method="GET" action="{{ route('reports.students') }}" class="mb-6 space-y-4" x-data="{ mode: '{{ request('mode', 'daily') }}' }">
                    <div class="flex space-x-4 mb-4">
                        <label class="inline-flex items-center">
                            <input type="radio" name="mode" value="daily" x-model="mode" class="form-radio text-purple-600">
                            <span class="ml-2">Harian</span>
                        </label>
                        <label class="inline-flex items-center">
                            <input type="radio" name="mode" value="monthly" x-model="mode" class="form-radio text-purple-600">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <template x-if="mode === 'daily'">
                            <div class="contents">
                                <div>
                                    ...
                                    <input type="date" id="start_date" name="start_date" value="{{ request('start_date', date('Y-m-d')) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5">
                                </div>
                            </div>
                        </template>
                        ...
                        <template x-if="mode === 'monthly'">
                            <div class="contents">
                                <div>
                                    ...
                                    <select id="month" name="month" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5">
                                ...
                                <div>
                                    ...
                                    <select id="year" name="year" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5">
                        ...
                        <div>
                            ...
                            <select id="class_id" name="class_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5">
                    ...
                    <div class="mt-4 flex justify-end">
                         <button type="submit" class="text-white bg-purple-600 hover:bg-purple-700 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 flex items-center focus:outline-none">
                ...
                <div class="flex space-x-2 justify-end mb-4 no-print">
                    <a href="{{ route('reports.students.export', request()->all()) }}" class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 flex items-center focus:outline-none">
                ...
                @if($mode === 'daily')
                 <div class="flex space-x-2 justify-end mb-4 no-print">
                    <a href="{{ route('reports.students.export-daily', request()->all()) }}" class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 flex items-center focus:outline-none">
                        <span class="material-icons text-sm mr-2">description</span> Export Excel
                    </a>
                    <button onclick="printTable()" class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 flex items-center focus:outline-none">
                        <span class="material-icons text-sm mr-2">print</span> Cetak
                    ...
                    <table id="search-table" class="w-full text-sm text-center text-gray-500 table-auto border-collapse">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left">NIS</th>
        ...
        <div class="relative w-full max-w-md max-h-full">
            <div class="relative bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 uppercase">
                ...
                <div class="px-6 py-6 lg:px-8">
                    <h3 class="mb-4 text-xl font-medium text-gray-900 ">Update Status Kehadiran</h3>
                    <form class="space-y-6" action="{{ route('reports.students.update') }}" method="POST">
                        @csrf
                        <input type="hidden" name="date" value="{{ $date ?? date('Y-m-d') }}">
                        <input type="hidden" name="student_id" id="modal_student_id">
                        
                        <div>
                            <label for="modal_student_name" class="block mb-2 text-sm font-medium text-gray-900 ">Nama Siswa</label>
                            <input type="text" id="modal_student_name" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 cursor-not-allowed" disabled>
                        </div>
    ...
    <script>
        function openModal(studentId, studentName, attendanceCodeId, shiftId, checkIn, note) {
            ...
            if (attendanceCodeId) {
                document.getElementById('modal_attendance_code').value = attendanceCodeId;
                 ...
                 // Default to first option and trigger logic
            ...
            toggleInputs(); // Initialize visibility based on current selection
        ...
        function toggleInputs() {
            ...
            if (statusName === 'Hadir') {
                ...
                // Check if user wants 'required' for other statuses. Assuming yes since asked.
                // "untuk inputan keterangan tambahkan attribut required jika status kehadiran selain dari status hadir"
